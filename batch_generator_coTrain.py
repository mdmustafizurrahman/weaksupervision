        ...
        for seed in seed_set:
            for batch in batch_set:
                for random_seed in random_seed_set:
                    #var = (variation-1)%mod
                    s = s + "\nTHEANO_FLAGS=device=gpu,floatX=float32 python " + "activeCo-TrainCNN.py " + str(sample) + " " + str(replica) + " " + str(seed) + " " + str(batch) + " " + str(random_seed)

                    #if variation%mod == 0:
                    tmp = '#!/bin/bash\n' \
                          '#SBATCH -J activeCoTrainJob' + str(variation) + '  # job name\n' \
                          '#SBATCH -o activeCoTrainJob' + str(
                    ...
                    shellcommand = shellcommand + '\nsbatch activeCoTrainJob'+ str(variation)
